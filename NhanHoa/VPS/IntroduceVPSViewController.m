...
@interface IntroduceVPSViewController ()<UIScrollViewDelegate, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource>{
...
@implementation IntroduceVPSViewController
@synthesize scvContent, bgHeader, lbTitle, viewHeader, icBack, lbHeader, icCart, lbCount, viewWindowsVPS, imgWindowsVPS, lbWindowsVPS, viewLinuxVPS, imgLinuxVPS, lbLinuxVPS, clvPromotions, clvSliders, tbQuestions;
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    lbHeader.text = [appDelegate.localization localizedStringForKey:@"SSD Cloud VPS"];
...
- (void)setupUIForView
{
    ...
    //  view content
    viewWindowsVPS.layer.cornerRadius = viewLinuxVPS.layer.cornerRadius = 8.0;
    
    lbWindowsVPS.font = lbLinuxVPS.font = [UIFont fontWithName:RobotoMedium size:textFont.pointSize-3];
    lbWindowsVPS.textColor = lbLinuxVPS.textColor = GRAY_80;
    ...
    [lbWindowsVPS mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(imgWindowsVPS.mas_bottom).offset(5.0);
        make.left.equalTo(viewWindowsVPS).offset(2.0);
        make.right.equalTo(viewWindowsVPS).offset(-2.0);
    }];
    
    UITapGestureRecognizer *tapOnLinuxVPS = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(whenTapOnLinuxVPS)];
    [viewLinuxVPS addGestureRecognizer: tapOnLinuxVPS];
    [viewLinuxVPS mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.bottom.equalTo(viewWindowsVPS);
    ...
    [lbLinuxVPS mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(imgLinuxVPS.mas_bottom).offset(5.0);
        make.left.equalTo(viewLinuxVPS).offset(2.0);
        make.right.equalTo(viewLinuxVPS).offset(-2.0);
    }];
    ...
    [lbTitle mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(viewWindowsVPS.mas_top).offset(-padding);
    ...
    [clvPromotions mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(viewWindowsVPS.mas_bottom).offset(padding);
        make.left.equalTo(viewWindowsVPS);
        make.right.equalTo(viewLinuxVPS);
...
- (void)whenTapOnWindowsVPS {
    
}

- (void)whenTapOnLinuxVPS {
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    lbSection.text = @"Hỏi đáp về máy chủ ảo VPS";
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (collectionView == clvPromotions) {
        ...
        cell.lbTitle.text = @"Vì sao nên thuê máy chủ ảo VPS Nhân Hòa";
