...
- (IBAction)btnUpdatePress:(UIButton *)sender {
    ...
    [self performSelector:@selector(goToUpdateScreen) withObject:nil afterDelay:0.01];
}
...
- (void)displayProfileInformation {
    ...
    NSString *frontImg = [profileInfo objectForKey:@"cmnd_a"];
    if (![AppUtils isNullOrEmpty: frontImg]) {
        [imgPassportFront sd_setImageWithURL:[NSURL URLWithString:frontImg] placeholderImage:FRONT_EMPTY_IMG];
    }else{
        imgPassportFront.image = FRONT_EMPTY_IMG;
    }
    
    NSString *behindImg = [profileInfo objectForKey:@"cmnd_b"];
    if (![AppUtils isNullOrEmpty: behindImg]) {
        [imgPassportBehind sd_setImageWithURL:[NSURL URLWithString:behindImg] placeholderImage:BEHIND_EMPTY_IMG];
    }else{
...
- (void)setupUIForView {
    ...
    float smallSize = [AppUtils getSizeWithText:@"Họ tên đầy đủ" withFont:[AppDelegate sharedInstance].fontRegular].width + 10.0;
    ...
    lbType.font = lbFullname.font = lbBOD.font = lbPassport.font = lbAddress.font = lbPhone.font = lbEmail.font = lbPassportFront.font = lbPassportBehid.font = [AppDelegate sharedInstance].fontRegular;
    
    lbTypeValue.font = lbFullnameValue.font = lbBODValue.font = lbPassportValue.font = lbAddressValue.font = lbPhoneValue.font = lbEmailValue.font = [AppDelegate sharedInstance].fontMedium;
