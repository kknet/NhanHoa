...
- (void)setupUIForView {
    ...
    tfPermanentAddr.returnKeyType = UIReturnKeyDone;
    ...
    [lbFront mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(lbBotCity);
        make.right.equalTo(lbBotCity.mas_centerX).offset(-padding/2);
        make.height.mas_equalTo(hLabel);
    }];
    ...
    lbFullname.font = lbGender.font = lbDOB.font = lbPassport.font = lbPhoneNumber.font = lbEmail.font = lbPermanentAddr.font = lbCountry.font = lbCity.font = mediumFont;
    tfFullname.font = lbMale.font = lbFemale.font = tfDOB.font = tfPassport.font = tfPhoneNumber.font = tfEmail.font = tfPermanentAddr.font = tfCountry.font = tfCity.font = textFont;
    
    lbFullname.textColor = lbGender.textColor = lbDOB.textColor = lbPassport.textColor = lbPhoneNumber.textColor = lbEmail.textColor = lbPermanentAddr.textColor = lbCountry.textColor = lbCity.textColor = GRAY_50;
    tfFullname.textColor = lbMale.textColor = lbFemale.textColor = tfDOB.textColor = tfPassport.textColor = tfPhoneNumber.textColor = tfEmail.textColor = tfPermanentAddr.textColor = tfCountry.textColor = tfCity.textColor = GRAY_80;
    
    lbBotFullname.backgroundColor = lbBotDOB.backgroundColor = lbBotPassport.backgroundColor = lbBotPhoneNumber.backgroundColor = lbBotEmail.backgroundColor = lbBotPermanentAddr.backgroundColor = lbBotCountry.backgroundColor = lbBotCity.backgroundColor = GRAY_220;
    
    float hContentView = padding + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hTextfield + 1.0) + paddingY + (hLabel + hPassport) + 2*paddingY + hBTN + 2*paddingY;
...
- (void)whenTapOnBacksideImage {
    if ([delegate respondsToSelector:@selector(clickOnBacksidePersonalProfile)]) {
        [delegate clickOnBacksidePersonalProfile];
        //  [delegate onPassportBehindPress];
    }
}

- (void)whenTapOnFrontImage {
    if ([delegate respondsToSelector:@selector(clickOnFrontPersonalProfile)]) {
        [delegate clickOnFrontPersonalProfile];
    }
}
...
- (void)displayInfoForPersonalProfileWithInfo: (NSDictionary *)info {
    NSString *fullname = [info objectForKey:@"cus_realname"];
    tfFullname.text = (![AppUtils isNullOrEmpty: fullname])? fullname : @"";
    ...
    NSString *dateOfBirth = [info objectForKey:@"cus_birthday"];
    tfDOB.text = (![AppUtils isNullOrEmpty: dateOfBirth])? dateOfBirth : @"";
    
    NSString *passport = [info objectForKey:@"cus_idcard_number"];
    tfPassport.text = (![AppUtils isNullOrEmpty: passport])? passport : @"";
    
    NSString *phone = [info objectForKey:@"cus_phone"];
    tfPhoneNumber.text = (![AppUtils isNullOrEmpty: phone])? phone : @"";
    ...
    tfPermanentAddr.text = (![AppUtils isNullOrEmpty: address])? address : @"";
...
- (IBAction)btnSaveInfoPress:(UIButton *)sender {
    ...
    if ([AppUtils isNullOrEmpty: tfEmail.text]) {
        [self makeToast:[[AppDelegate sharedInstance].localization localizedStringForKey:@"Please enter email"] duration:2.0 position:CSToastPositionCenter style:[AppDelegate sharedInstance].errorStyle];
        return;
    }
...
- (void)addDatePickerForViewWithFont: (UIFont *)textFont {
    ...
    [btnClose setTitle:[[AppDelegate sharedInstance].localization localizedStringForKey:@"Close"]
              forState:UIControlStateNormal];
    ...
    [btnChoose setTitle:[[AppDelegate sharedInstance].localization localizedStringForKey:@"Choose"]
               forState:UIControlStateNormal];
...
- (void)removePassportFrontPhoto {
    ...
    if (![AppUtils isNullOrEmpty: linkFrontPassport]) {
        [imgFront sd_setImageWithURL:[NSURL URLWithString:linkFrontPassport] placeholderImage:FRONT_EMPTY_IMG];
    }else{
        imgFront.image = FRONT_EMPTY_IMG;
    }
}
...
- (void)removePassportBacksidePhoto {
    ...
    if (![AppUtils isNullOrEmpty: linkBehindPassport]) {
        [imgBackside sd_setImageWithURL:[NSURL URLWithString:linkFrontPassport] placeholderImage:BEHIND_EMPTY_IMG];
    }else{
        imgBackside.image = BEHIND_EMPTY_IMG;
    }
}
...
- (void)saveAllValueBeforeChangeView {
    ...
    [[AppDelegate sharedInstance].profileEdit setObject:tfPermanentAddr.text forKey:@"cus_address"];
    [[AppDelegate sharedInstance].profileEdit setObject:cityCode forKey:@"cus_city"];
}

#pragma mark - UITextfield delegate
-(BOOL)textFieldShouldReturn:(UITextField *)textField {
    if (textField == tfPassport) {
...
-(void)failedToEditProfileWithError:(NSString *)error {
    ...
    NSString *content = [AppUtils getErrorContentFromData: error];
    if ([delegate respondsToSelector:@selector(failedToUpdatePersonalProfileWithError:)]) {
        [delegate failedToUpdatePersonalProfileWithError: content];
    }
}
...
-(void)editProfileSuccessful {
    ...
    if ([delegate respondsToSelector:@selector(updatePersonalProfileSuccessfully)]) {
        [delegate updatePersonalProfileSuccessfully];
    }
}
