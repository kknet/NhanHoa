...
@synthesize imgType, lbSepa, lbDomain, lbPrice, lbOldPrice, viewParent, padding, hItem, lbPriceLine, paddingX;
...
- (void)awakeFromNib {
    ...
    [lbPrice mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(viewParent.mas_centerY);
    ...
    [lbDomain mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(lbPrice.mas_top);
    }];
    
    lbOldPrice.font = [AppDelegate sharedInstance].fontRegular;
...
- (void)showOldPriceForCell: (BOOL)show {
    if (show) {
        [lbPrice mas_remakeConstraints:^(MASConstraintMaker *make) {
            make.centerY.equalTo(viewParent.mas_centerY);
            make.left.equalTo(lbSepa.mas_right).offset(10.0);
            make.right.equalTo(viewParent).offset(-5.0);
            make.height.mas_equalTo(25.0);
        }];
